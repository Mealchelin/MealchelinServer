    ...
    <main>
        ...
        <section id="pd-section1">
            <div class="pd-main-navi">
           		<h3 class="pd-main-heading">'${ result }'에 대한 검색 결과입니다.</h3>
           		<div class="pd-main-search" style="margin: 20px 0;">
        ...
        <section id="pd-section2" style="margin-bottom: 80px">
        	<c:if test="${ listCount == 0 }">
           		<h3 style="margin: 200px auto; width: fit-content;">검색 결과가 없습니다.</h3>
<%--             		<jsp:include page="" /> --%>
           	</c:if>
        ...
        <c:if test="${ listCount != 0 }">
	        <section id="pd-section3">
	            <div class="pd-paging">
	                <!-- 이전 페이지로 -->
					<button onclick="location.href='${ path }/product/search?${ result }page=${ pageInfo.prevPage }'">&lt;</button>
					...
					<c:forEach var="current" begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }">
						<c:choose>
							...
							<c:otherwise>
								<button onclick="location.href='${ path }/product/search?${ result }page=${ current }'">${ current }</button>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					
					<!-- 다음 페이지로 -->
					<button onclick="location.href='${ path }/product/search?${ result }page=${ pageInfo.nextPage }'">&gt;</button>
	            </div>
	        </section>
        </c:if>
    </main>
