...
document.getElementById("payButton").addEventListener("click", function () {
    ...
    let productsCountele = $('.pay_number');
    let productsCount = new Array();
    for(let i = 0; i< productsCountele.length; i++)
    {
        productsCount.push(productsCountele[i].innerText);
    }
    //paypaynumber
    let priceEle = $('input[name=paypaynumber]');
    let prices = new Array();
    for(let i = 0; i< priceEle.length; i++)
    {
        prices.push(priceEle[i].value);
    }
    let prdNosele = $('input[name=prdNo]');
    let prdNos   = new Array();
    for(let i = 0; i< prdNosele.length; i++)
    {
        prdNos.push(prdNosele[i].value);
    }

    let orderPrdData = 
    {
        orderNo : payData.merchant_uid,
        productsCount,
        prdNos,
        prices
    };
    ...
    $.ajax({
        type: "POST",
        url: "/payment/saveOrderProduct",
        data: JSON.stringify(orderPrdData),
