...
function getProductAdress() {
    var productAddr = $('.shipaddress').text(); // 상품명을 클래스 이름이 'pay_FirstName'인 요소에서 가져옴
    var productSubAddr = $('.shipaddressdetail').text(); // 브랜드명을 클래스 이름이 'pay_subFirstName'인 요소에서 가져옴
    return productAddr + ' ' + productSubAddr; // 상품명과 브랜드명을 결합하여 반환
}
 ...
 function openPaymentPage(method, price) {
    ...
    IMP.request_pay({
        ...
        if (rsp.success) {
            ...
            $.ajax({
                type: "POST",
                url: '{payment/paysucces}', // 결제 정보 전송을 처리하는 서버의 URL
                data: JSON.stringify(data), // 데이터를 JSON 문자열로 변환하여 전송
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function(response) {
